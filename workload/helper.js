...
const clearLedger = async (workloadModule) => {
  const myArgs = {
    ...
    contractArguments: [ENV.assetType, JSON.stringify([ENV.orgName, workloadModule.workerIndex.toString()])],
    readOnly: true
  };
...
const iwmCreateAssets = async (workloadModule) => {
  ...
  for (let i = 0; i < ENV.nAsset; i++) {
    ...
    const keys = [ENV.orgName, workloadModule.workerIndex.toString(), ...keysDate, i.toString()];
